...
function gradeQuickReply(
  ...
  const items: QuickReplyItem[] = gradeOptions.slice(0, 12).map(
    (option) =>
      ({
        type: "action",
        action: {
...
function attendanceDateQuickReply(dates: string[]): Message["quickReply"] {
  const items: QuickReplyItem[] = dates.slice(0, 4).map(
    (date) =>
      ({
        type: "action",
        action: {
...
function attendanceStatusQuickReply(): Message["quickReply"] {
  ...
  const items: QuickReplyItem[] = statuses.map(
    (status) =>
      ({
        type: "action",
        action: {
